...
server.tool(
    'ask_claude',
    {
        ...
        model: z.string().optional().describe('claude.ai model id. Defaults to claude-opus-5-5. Wire-confirmed ids: claude-opus-5-5, claude-opus-5, claude-sonnet-5. This is per-request and does NOT change the conversation stored model. An unavailable id returns 403.'),
...
server.tool(
    'ask_qwen',
    {
        ...
        mode: z.enum(['t2t', 'search', 'deep_research', 'artifacts', 'web_dev', 'learn', 'slides', 'travel', 't2i', 'image_edit'])
            .optional()
            .describe('Qwen chat_type. Default t2t. "deep_research" is a long-running multi-step research mode — budget MINUTES per call, not seconds. "t2i" generates an image; "image_edit" edits one you pass in `attachments`. Generated assets are DOWNLOADED to disk and their local paths returned, because the CDN URLs are signed and expire — never hand a caller the URL. NO VIDEO MODE: asking Qwen for a video in plain language gets a flat refusal (it names Runway/Pika/Sora instead), and real video generation is image-anchored `i2v`, which is a different transport this tool does not implement yet — non-streaming, plus polling for 4-6 minutes. Switching mode starts a fresh Qwen conversation because chat_type is fixed when the conversation is created.')
    },
